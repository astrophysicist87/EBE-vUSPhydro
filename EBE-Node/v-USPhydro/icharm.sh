...
do 
   ...
   mv outputfiles/"$t"/"$dec""$4"/"$5"/freezeout_ev"$m".dat    df/input/"$t"/"$dec""$4"/"$5"/freezeout_ev"$m".dat
   cd df
   ./fo "$6"input"$1".dat "$1" "$m" 0 0 0
   ./decays/reso  out/"$t"/"$dec""$4"/"$5"/ev"$m"i_dNdphidpp.dat out/"$t"/"$dec""$4"/"$5"/ev"$m"di_dNdphidpp.dat reso16p.inp
   echo decays done
   cd ..
done
