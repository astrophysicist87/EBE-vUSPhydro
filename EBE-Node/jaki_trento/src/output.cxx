...
void write_stream(std::ostream& os, int width,
  ...
  os << setprecision(10)
     ...
     << setw(5)                 << event.npart()
     << setw(18) << fixed << event.multiplicity()
     << fixed;

  for (const auto& ecc : event.eccentricity())
    os << setw(14)             << ecc.second;
  for (int nn=0;nn<5;nn++)  os << " " << event.cgcvn_[nn];
  os << '\n';
}
...
void write_text_vUSPhydro(const fs::path& output_dir, int width,
    int num, double impact_param, const Event& event, bool header) {
...
//        << "# mult  = " << event.multiplicity() << '\n';
    ...
    ofs << num << " " << event.dxy_ << " " << event.dxy_ << " " << event.multiplicity() << " " << -event.nsteps_*event.dxy_/2 << " " << -event.nsteps_*event.dxy_/2 <<  '\n'; 
...
//      ofs << "# e" << ecc.first << "    = " << ecc.second << '\n';
  ...
  // increases output speed and saves disk space since many grid elements are
  // zero.
  double xcoor=-event.nsteps_*event.dxy_/2;
  double ycoor=-event.nsteps_*event.dxy_/2;
  for (const auto& row : event.reduced_thickness_grid()) {
    auto&& iter = row.begin();
    // Write all row elements except the last with a space delimiter afterwards.
    do {
      if(*iter>0) ofs << xcoor << " " << ycoor << " " <<  *iter << '\n';
      ycoor+=event.dxy_;
    } while (++iter != --row.end());
	
    // Write the last element and a linebreak.
    if(*iter>0) ofs << xcoor << " " << ycoor << " " <<  *iter << '\n';
    ycoor=-event.nsteps_*event.dxy_/2;
    xcoor+=event.dxy_;
  }
}
