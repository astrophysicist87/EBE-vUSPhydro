...
NucleusPtr Nucleus::create(const std::string& species, double nucleon_dmin) {
  ...
  if (species == "p")
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
       62, 4.20, 0.596, 0.162, 0, -0.006, nucleon_dmin
    }};
  else if (species == "Au")
    return NucleusPtr{new WoodsSaxonNucleus{
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
	9, 1.4, 0.75, 0, 0.64,0, 0.27, nucleon_dmin
	  }};
  else if (species == "Xe")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      129, 5.42, 0.57, 0, 0.162,0, -0.003, nucleon_dmin
    }};
  else if (species == "Xehalf")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
	129, 5.42, 0.57, 0, 0.081,0, -0.003, nucleon_dmin
	  }};
  else if (species == "Xeb2_25")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
	129, 5.42, 0.57, 0, 0.2025,0, -0.003, nucleon_dmin
	  }};
  else if (species == "Xeb2_36")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        129, 5.42, 0.57, 0, 0.22,0, -0.003, nucleon_dmin
          }};
  else if (species == "Xeb2_50")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        129, 5.42, 0.57, 0, 0.3,0, -0.003, nucleon_dmin
          }};
  else if (species == "Xeb4_25")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        129, 5.42, 0.57, 0, 0.162,0, -0.00375, nucleon_dmin
          }};
  else if (species == "Xeb2b4_25")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        129, 5.42, 0.57, 0, 0.2025,0, -0.00375, nucleon_dmin
          }};
  else if (species == "Au2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      197, 6.38, 0.535,  0, -0.131,0, -0.031, nucleon_dmin
    }};
  else if (species == "Pb")
    return NucleusPtr{new WoodsSaxonNucleus{
    ...
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      208, 6.62, 0.546,  0, 0., 0.0375,0., nucleon_dmin
    }};
  else if (species == "Pb3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      208, 6.62, 0.546, 0, 0., 0.12,0., nucleon_dmin
    }};
  else if (species == "Pb4")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      208, 6.62, 0.546, 0, 0., 0.075,0., nucleon_dmin
    }};
  else if (species == "Pbb3b4")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
	208, 6.62, 0.546, 0, 0., 0.075,0.075, nucleon_dmin
	  }};
  else if (species == "U")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.81, 0.600, 0, 0.280, 0,0.093, nucleon_dmin
    }};
  else if (species == "U2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.86, 0.420,  0, 0.265,0, 0.000, nucleon_dmin
    }};
  else if (species == "U3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      238, 6.67, 0.440,  0, 0.280, 0,0.093, nucleon_dmin
    }};
  else if (species == "UD")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        238, 6.81, 0.600, 0, 0.200, 0,0.098, nucleon_dmin
          }};
  else if (species == "UE")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
	238, 6.81, 0.600, 0, 0.236, 0,0.098, nucleon_dmin
	  }};
  else if (species == "UF")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        238, 6.62, 0.600, 0, 0.236, 0,0.098, nucleon_dmin
          }};
  else if (species == "UG")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        238, 6.81, 0.500, 0, 0.236, 0,0.098, nucleon_dmin
          }};
  else if (species == "UH")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        238, 6.62, 0.500, 0, 0.236, 0,0.098, nucleon_dmin
          }};
  else if (species == "Umus")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        238, 6.81, 0.550, 0, 0.28, 0,0.093, nucleon_dmin
          }};
  else if (species == "Ncon_0")
    return NucleusPtr{new WoodsSaxonNucleus{
      20000, 6.62, 0.546, nucleon_dmin
    }};
  else if (species == "Ncon_075")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      20000, 6.62, 0.546,  0, 0., 0.075,0., nucleon_dmin
    }};
  else if (species == "Ar_3pF")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      40, 3.73, 0.621,  -0.191, 0, 0,0, nucleon_dmin
    }};
  else if (species == "Ar_2pF")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      40, 3.53, 0.542, 0, 0, 0,0, nucleon_dmin
    }};
  else if (species == "O_3pF")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
      16, 2.608, 0.513,  -0.051, 0, 0,0, nucleon_dmin
    }};
  else if (species == "RuA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.085, 0.460, 0, 0.1, 0,0.0, nucleon_dmin
          }};
  else if (species == "RuB")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.085, 0.460, 0, 0.158, 0,0.0, nucleon_dmin
          }};
  else if (species == "RuC")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.085, 0.460, 0, 0.2, 0,0.0, nucleon_dmin
          }};
  else if (species == "ZrA")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.02, 0.460, 0, 0.0, 0,0.0, nucleon_dmin
          }};
  else if (species == "ZrB")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.02, 0.460, 0, 0.1, 0,0.0, nucleon_dmin
          }};
  else if (species == "ZrC")
    return NucleusPtr{new DeformedWoodsSaxonNucleus{
        96, 5.02, 0.460, 0, 0.2, 0,0.0, nucleon_dmin
          }};
...
DeformedWoodsSaxonNucleus::DeformedWoodsSaxonNucleus(
    std::size_t A, double R, double a, double omega, double beta2, double beta3,  double beta4, double dmin)
    : MinDistNucleus(A, dmin),
      R_(R),
      a_(a),
      om_(omega),
...
double DeformedWoodsSaxonNucleus::deformed_woods_saxon_dist(
  ...
  auto Y30 = std::sqrt(7)/4. * one_div_root_pi * (5.*std::pow(cos_theta,3.) - 3.*cos_theta);
  auto Y40 = 3./16. * one_div_root_pi *
             (35.*cos_theta_sq*cos_theta_sq - 30.*cos_theta_sq + 3.);


  // "effective" radius
  auto Reff = R_ * (1. + beta2_*Y20 + beta3_*Y30+ beta4_*Y40);

  return 1. / (1. + std::exp((r - Reff) / a_));
}

double DeformedWoodsSaxonNucleus::deformed_woods_saxon_dist3F(
    double r, double cos_theta) const {
  auto cos_theta_sq = cos_theta*cos_theta;
  ...
  return (1.+om_*pow(r,2)/pow(Reff,2)) / (1. + std::exp((r - Reff) / a_));
}
...
void DeformedWoodsSaxonNucleus::sample_nucleons_impl() {
  ...
  std::string name="def_angles.dat";
  std::ofstream PRINT(name.c_str(), std::ios_base::app | std::ios_base::out);
  if (!PRINT.is_open())
    {
      std::cout << "Can't open " << name << std::endl;
      exit(1);
    }
  PRINT << ang1 << " " << ang2  << std::endl;
  ...
  for (auto&& sample : samples) {
    ...
    do {
      ...
      random::canonical<double>() >
      deformed_woods_saxon_dist3F(sample.r, sample.cos_theta)
