...
inline double geometric_mean(double a, double b) {
  ...
  // dilue-dilute regime p==42
  inline double dilutedilute(double a, double b) {
    return a*b;
  }
  ...
  if (std::fabs(p) < TINY) {
    ...
    compute_reduced_thickness_ = [this, p]() {
      compute_reduced_thickness(
				[p](double a, double b) { return  dilutedilute(a, b); });
    };
  } else if (p > 0.) {
    compute_reduced_thickness_ = [this, p]() {
      compute_reduced_thickness(
...
void Event::compute_reduced_thickness(GenMean gen_mean) {
  ...
  for (int iy = 0; iy < nsteps_; ++iy) {
    for (int ix = 0; ix < nsteps_; ++ix) {
      ...
      for (int nn = 0; nn < 5; ++nn) cv[nn]+= pow(TA_[iy][ix]*TB_[iy][ix],nn+1);
  ...
  cgcvn_[0]= dxy_ * dxy_ *cv[0];
  for (int nn = 1; nn < 5; ++nn) cgcvn_[nn]= dxy_ * dxy_ *cv[nn]/cgcvn_[0];
  ixcm_ = ixcm / sum;
  iycm_ = iycm / sum;
}
