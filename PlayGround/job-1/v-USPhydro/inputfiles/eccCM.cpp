...
double rn(vector <double> rx,vector <double> ry, vector <double> e, int m);
...
int main (int argc, char *argv[]) //execute with ./a.out start_event# end_event#
{
	...
	vector< double > ec2,rad,psi2,ec3,psi3,ec4,psi4,ec5,psi5,ec6,psi6,ec15,psi15,ec33,psi33,ec34,psi34,ec35,psi35,ec36,psi36,ec11,psi11,ec12,psi12,ec44,psi44,ec13,psi13;
        vector<double> ec55,psi55,ec66,psi66,r22,r3,r4,r5,r6;
	ec2.resize(evtot);
	rad.resize(evtot);
	psi2.resize(evtot);
	ec3.resize(evtot);
...
//	phi.push_back(atan2(y[1],y[0]) );
	...
	psi13[ev]=psisb;
        ...
        ec55[ev]=ecc(psisb,rx,ry,r2,phi,e,5,5,rads);
        psi55[ev]=psisb;

        ec66[ev]=ecc(psisb,rx,ry,r2,phi,e,6,6,rads);
        psi66[ev]=psisb;	 
	...
	for(int ev=ev1;ev<=ev2;ev++){
	  ...
	  OUT2 <<  ec2[ev] << " " << psi2[ev] <<  " " <<   ec3[ev] << " " << psi3[ev] <<  " " <<  ec4[ev] << " " << psi4[ev] <<  " " <<  ec5[ev] << " " << psi5[ev] <<  " " <<  ec6[ev] << " " << psi6[ev] <<  " " <<  ec15[ev] << " " << psi15[ev] <<  " " <<  ec33[ev] << " " << psi33[ev] <<  " " <<  ec34[ev] << " " << psi34[ev] <<  " " <<  ec35[ev] << " " << psi35[ev] <<  " " <<  ec36[ev] << " " << psi36[ev] <<    " " <<   ec12[ev] << " " << psi12[ev] <<  " " << ec44[ev] << " " << psi44[ev] << " " << ec13[ev] << " " << psi13[ev] <<   " " << rad[ev] << " " << r22[ev] << " " << r3[ev] << " " << r4[ev] <<  " " << r5[ev] <<  " " << r6[ev] << " " << ec55[ev] << " " <<psi55[ev]<< " "  << ec66[ev] << " " << psi66[ev] <<    endl;
	}
